...
#define D_MESSAGE_ITEM_GENERATION       (DEBUGGING && 1)
...
#define FIRST_LEVEL             1           // first level (is 1, duh) -- gsr
#define GUARANTEED_VAULT_LEVEL  1
...
//	RING_LIGHT,
//	RING_AWARENESS,
	RING_PERCEPTION, // Rolled the two above into one -- gsr
	RING_WISDOM,
    RING_REAPING,
    RING_PROPULSION,
    RING_SPEED,
...
typedef struct creature {
	creatureType info;
	short uniqueID; // gsr
    char called[COLS]; // TO DO: be able to name monsters something -- gsr
	short xLoc;
	short yLoc;
	short depth;
	short currentHP;
